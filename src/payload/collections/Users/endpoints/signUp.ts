...
export const signUp: PayloadHandler = async (req: PayloadRequest, res) => {
  ...
  const checkoutSession = await stripe.checkout.sessions.create({
    ...
    success_url:
      `${process.env.NEXT_PUBLIC_SERVER_URL}/api/users/success?session_id={CHECKOUT_SESSION_ID}&name=${encodeURIComponent(req.body.name)}&email=${encodeURIComponent(req.body.email)}&password=${encodeURIComponent(req.body.password)}&phone=${req.body.phone}` +
      (req.body.referralCode ? `&referral=${req.body.referralCode}` : ''),
